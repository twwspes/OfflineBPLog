...
interface BloodPressureTypeForStatistics {
...
interface BloodPressureTypeForAggregatedResult {
...
export const fetchBloodPressureFromSQL = async () => {
  ...
  return allRows as Array<BloodPressureType>;
};
...
export const fetchBloodPressureFromSQLBtwDateMilli = async (
  ...
  if (sample !== null && sample !== undefined && sample === 1) {
    ...
    return allRows as Array<BloodPressureTypeForStatistics>;
  }
  if (sample !== null && sample !== undefined && sample > 1) {
    ...
    if (pureresultRows.length < 24) {
      return pureresultRows as Array<BloodPressureType>;
    }
  ...
  if (limit >= 0 && offset >= 0) {
    ...
    return resultRows as Array<BloodPressureType>;
  }
  const resultRows = await (
    await db
  ).getAllAsync(
