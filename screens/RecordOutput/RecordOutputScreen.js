...
const RecordOutputScreen = (props) => {
  ...
  const privacyPolicyWebsite = locale2.includes('zh')
  ...
  useEffect(() => {
    const downloadItems = async () => {
      ...
      try {
        ...
        setBloodPressures([]);
        console.log(
          'failed to download RecordOutputScreen useEffect downloadItems',
        );
        setIsLoading(false);
    ...
    Alert.alert(t('warning'), t('all_logs_removed_warning'), [
      {
        ...
        onPress: async () => {
          ...
          const promise = dispatch(
            bloodPressureActions.deleteAllBloodPressures(),
          );
          promise.then(() => {
            setIsLoading(false);
          });
                          ...
                          if (
                            ...
                            const remark =
                              item.Remark !== undefined
                                ? item.Remark.toString().substring(0, 300)
                                : '';

                            // console.log("RecordOutputScreen date", date, bloodPressuresReverseIndex);
                            if (
                              bloodPressuresReverseIndex <
                              bloodPressuresReverse.length
                            ) {
                              for (
                                let i = bloodPressuresReverseIndex;
                                i < bloodPressuresReverse.length;
                                i++
                              ) {
                                // console.log(date);
                                ...
                                if (
                                  date.valueOf() ===
                                  Math.floor(
                                    Number(bloodPressuresReverse[i].id) / 1000,
                                  ) *
                                    1000
                                ) {
          ...
          <View style={styles.buttonContainer}>
            <MainButton
              onPress={async () => {
                ...
                const bloodPressuresJson = sampleBloodPressureLog.map(
                  (item) => {
                    const dateTemp = new Date(parseInt(item.id));
                    return {
                      systolic: item.systolic_blood_pressure,
